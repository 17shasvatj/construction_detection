"""
Step 4: Validate label quality — NDVI separation, density, trajectory, DW distribution.

Standalone: python -m pipeline.validate_labels --aoi babcock
"""
...
def run(data_dir):
    ...
    # Confirmed pixels: traversed grading phase then construction phase
    ...
    # ── 2. Development-zone density check ────────────────────────────────
    ...
    if confirmed.sum() > 0:
        ...
        for t in range(T):
            ...
            print(f"    {quarters[t]:<12}  {np.nanmean(vals):>10.3f}  {np.nanstd(vals):>8.3f}")
    else:
        print("    No confirmed pixels found.")

    # ── 4. DW class distribution of confirmed pixels ──────────────────────
    key_classes = [1, 2, 5, 6, 7]  # trees, grass, shrub, built, bare
    print(f"\n[4] DW class distribution for confirmed pixels (% per quarter):")
    if confirmed.sum() > 0:
        header = f"    {'Quarter':<12}" + "".join(f"  {DW_NAMES[c]:<8}" for c in key_classes)
        print(header)
